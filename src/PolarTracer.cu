...
template <template<typename, Device> typename _PTRDST, typename TDST, Device DDST,
          template<typename, Device> typename _PTR_SRC, typename T_SRC, Device D_SRC>
__host__ __device__ inline void CopySize(const _PTRDST<TDST, DDST>& dst,
                                            const _PTR_SRC<T_SRC, D_SRC>& src,
                                            const size_t size) noexcept
{
    static_assert(std::is_same_v<TDST, T_SRC>, "Incompatible Source And Destination Raw Pointer Types");
    
    cudaMemcpyKind memcpyKind;

    if constexpr (D_SRC == Device::CPU && DDST == Device::CPU) {
        memcpyKind = cudaMemcpyKind::cudaMemcpyHostToHost;
    } else if constexpr (D_SRC == Device::GPU && DDST == Device::GPU) {
        memcpyKind = cudaMemcpyKind::cudaMemcpyDeviceToDevice;
    } else if constexpr (D_SRC == Device::CPU && DDST == Device::GPU) {
        memcpyKind = cudaMemcpyKind::cudaMemcpyHostToDevice;
    } else if constexpr (D_SRC == Device::GPU && DDST == Device::CPU) {
...
template <template<typename, Device> typename _PTRDST, typename TDST, Device DDST,
          template<typename, Device> typename _PTR_SRC, typename T_SRC, Device D_SRC>
__host__ __device__ inline void CopyCount(const _PTRDST<TDST, DDST>& dst,
                                            const _PTR_SRC<T_SRC, D_SRC>& src,
                                            const size_t count) noexcept
{
    static_assert(std::is_same_v<TDST, T_SRC>, "Incompatible Source And Destination Raw Pointer Types");

    CopySize(dst, src, count * sizeof(TDST));
}

template <template<typename, Device> typename _PTRDST, typename TDST, Device DDST,
            template<typename, Device> typename _PTR_SRC, typename T_SRC, Device D_SRC>
__host__ __device__ inline void CopySingle(const _PTRDST<TDST, DDST>& dst,
                                            const _PTR_SRC<T_SRC, D_SRC>& src) noexcept
{
    static_assert(std::is_same_v<TDST, T_SRC>, "Incompatible Source And Destination Raw Pointer Types");
    
    CopySize(dst, src, sizeof(TDST));
}
...
struct Vec4 {
    ...
    static __host__ __device__ inline Vec4<T> Normalized3D(const Vec4<T> v) noexcept {
      v.Normalize3D();
      return v;
    }

    static __host__ __device__ inline Vec4<T> Normalized4D(const Vec4<T> v) noexcept {
      v.Normalize4D();
      return v;
    }
}; // Vec4<T>
